...
Shader "Unity MyShader/GGX_Repetition"
{
    Properties
    {
        [Header(ShaderEnum)]
        ...
        */
        ...
        _SpecularGloss ("Specular Gloss",Range(0,256)) = 20
...
/*
               ...
               */
                ...
                //==========================GGX=================================
                //============ILM: R G B A,Intensity,AO,Coefficient,Outline
                //GGX:ShadowColor = albedo * SSS_TEX_Color * SceneColor;
                
                float isShadow = step(halfLambert,1) || step(lightMap.g * _AO,0.5);//have shadow when isShadow = 0
                float4 ShadowColor = float4(albedo,1) * sssTex * _ShadowColor;
                diffuse = _MainLightColor * lerp(ShadowColor,baseColor,isShadow);
                //由于ndotH可能为0，使得结果发生剧变，光的另一侧脸直接全黑
                //float I = 
                float3 specularColor = albedo * _SpecularColor;
                float spec = _SpecularIntensity * lightMap.b * (pow(ndotH, _SpecularGloss));
                float3 specular = ((spec < lightMap.r)?float3(0,0,0):specularColor);
                //float3 specular = ((_SpecularIntensity < lightMap.r)?float3(0,0,0):specularColor) * (pow(ndotH, _SpecularGloss * lightMap.b));
                
                return float4(diffuse + specular + rimLight,1);
